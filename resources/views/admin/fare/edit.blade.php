<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 mb-5">
            <x-link-button :href="route('fare.index')"
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden p-11 shadow-sm sm:rounded-lg">
                <form action="{{ empty($fare) ? route('fare.store') : route('fare.update', $fare->id) }}"
                    method="POSt">
                    @csrf
                        ...
                        <x-select id="base-location" name="base_location" required>
                            @foreach ($locations as $location)
                                <option value="{{ $location->id }}"
                                    {{ old('base_location', empty($fare) ? '' : $fare->base_location) == $location->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <x-select id="start-from" name="start_from" required>
                            @foreach ($locations as $location)
                                <option value="{{ $location->id }}"
                                    {{ old('start_from', empty($fare) ? '' : $fare->start_from) == $location->id ? 'selected' : '' }}>
                                    {{ $location->place_name }}</option>
                            @endforeach
                        </x-select>
                    ...
                    <div class="mb-3">
                        ...
                        <x-select id="destination" name="destination" required>
                            @foreach ($locations as $location)
                                <option value="{{ $location->id }}"
                                    {{ old('destination', empty($fare) ? '' : $fare->destination) == $location->id ? 'selected' : '' }}>
                                    {{ $location->place_name }}</option>
                            @endforeach
                        </x-select>
                    ...
                    <div class="mb-3">
                        ...
                        <x-text-input id="fare-amt" class="block mt-1 w-full" type="number" min="1"
                            step="0.01" name="fare_amt" :value="old('fare_amt', empty($fare) ? '' : $fare->fare_amt)" required />
                    ...
                    <div class="mb-3">
                        ...
                        <div class="relative">
                            ...
                            <x-text-input datepicker datepicker-autohide datepicker-format="yyyy-mm-dd" id="effect-from"
                                class="block mt-1 w-full ps-10" type="text" name="effect_from" :value="old('effect_from', empty($fare) ? date('Y-m-d') : $fare->effect_from)"
                                required />
                        </div>
